...
2. Protocol and serialization/de-serialization code for use over the uart
...
1. Clone the repository
   ...
   cd nuhal
   #+END_SRC
2. Build, test, and install, like a standard CMake Project
   #+BEGIN_SRC bash
   cmake -B build .
   cmake --build build
   ...
   #+END_SRC
3. Technically, ~nuhal_all~, ~nuhal_linux~, and ~nuhal_tiva~ are complete projects that can be built separately.
   - Build ~nuhal_all~ first becuase ~nuhal_linux~ and ~nuhal_tiva~ depend on it.
   - Use ~CMAKE_INSTALL_PREFIX~ to specify a directory to install each package so you don't need root
   - Use ~CMAKE_PREFIX_PATH~ to point the libraries to the proper location
   - The top-level ~CMakeLists.txt~ automates these separate build processes.  Each package must be built separately
     because a single ~CMake~ project can only use a one toolchain.
* Usage
