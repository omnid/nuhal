...
1. Clone the repository
   ...
   cd nuhal
   #+END_SRC
2. Build and test the core code
   #+BEGIN_SRC bash
   cmake -B nuhal_all/build nuhal_all
   cmake --build nuhal_all/build
   cmake --build nuhal_all/build --target test
   #+END_SRC
3. Install the core code either system-wide  or to a specified location
   1. System wide (requires root) ~cmake --build nuhal_all/build --target install~
   2. Locally (to ~<dir>~)  ~cmake --build nuhal_all/build --target install -DCMAKE_INSTALL_PREFIX=<dir>~
3. Repeat the above steps for each platform you wish to compile for
   - If you installed locally to ~<dir>~, you must add ~-DCMAKE_PREFIX_PATH=<dir>~ to the above commands
   - ~nuhal_linux~ contains the linux-specific code
   - ~nuhal_tiva~ contains TI Tiva microcontroller code. It depends on [[https://github.com/m-elwin/tiva-cmake][tiva-cmake]] being installed
* Usage
