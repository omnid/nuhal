...
2. Build and install ~nuhal_all~, code that is common to all platforms.
   ...
   cmake --build nuhal_all/build --target install
   #+END_SRC
...
6. To run the unit tests
   ...
   #+END_SRC
7. When generating the build-system (~cmake -B ...~) for ~nuhal_all~ you can set an install prefix by passing ~-DCMAKE_INSTALL_PREFIX=$INSTALL_DIR~, 
   where ~$INSTALL_DIR~ is the absolute path to the installation directory. The project to be installed to ~$INSTALL_DIR~. 
   You should then specify ~-DCMAKE_PREFIX_PATH=$INSTALL_DIR~ when generating the build-system for the platform specific libraries.
* Usage
